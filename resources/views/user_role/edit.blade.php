                ...
                <div class="card bg-secondary shadow">
                    <div class="card-header bg-white border-0">
                        <div class="row align-items-center">
                            <h3 class="mb-0">{{ __('Edit User Roles') }}</h3>
                        </div>
                    </div>
                    <div class="card-body">
                        <form method="post" action="{{ route('roleuser.update', $roleuser->id) }}" autocomplete="off">
                            @csrf
                            @method('PUT')
                            ...
                            <div class="row">
                                <div class="col-lg-6 col-md-6 col-sm-12">
                                    <div class="form-group">
                                        ...
                                        <input type="text" value="{{$roleuser->users->name}}" class="form-control autosuggest ui-widget"
                                            placeholder="{{ __('name') }}" readonly>
                                        <input type="text" name="name" value="{{$roleuser->user_id}}" hidden>
                                    </div>
                                </div>
                                <div class="col-lg-6 col-md-6 col-sm-12">
                                    <div class="form-group{{ $errors->has('for') ? ' has-danger' : '' }}">
                                        ...
                                        <select name="for" class="form-control form-control-alternative"
                                            ...
                                            <option value="{{$roleuser->role_id}}" selected>{{$roleuser->roles->name}}</option>
                                            @foreach ($roles as $x)
